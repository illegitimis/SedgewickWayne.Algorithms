...
public class BinarySearchTests
{
    ...
    public BinarySearchTests(ITestOutputHelper output)
    {
        ...
        var tasks = (new string[] { largew, larget })
                .Select(x => {
                    // Task.Run(() => Helper.DownloadFile(new Uri($"http://algs4.cs.princeton.edu/11model/{x}"), x))
                    var url = $"http://algs4.cs.princeton.edu/11model/{x}";
                    var uri = new Uri(url, UriKind.Absolute);
                    return Helper.DownloadFileAsync(uri, x, percent => _output.WriteLine($"[{x}] Progress {percent}"));
                }).ToArray();
        Task.WaitAll(tasks);
